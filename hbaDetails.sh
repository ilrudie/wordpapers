...
then
	#print header
	printf "%12s %4s %6s %3s %8s %14s" "Model" "LQD" "QD" "TMO" "FW Rev" "Driver" 2>/dev/null
	return
...
#print emulex detail
printf "%12s %4s %6s %3s %8s %14s" $MODELNAME $LQD $QD $TMO $FWREV $DRREV 2>/dev/null
...
printf "%6s %24s %8s %8s" "HBA" "WWPN" "State" "Speed" 2>/dev/null
#detect emulex cards
...
do
	...
	printf "%6s %24s %8s %8s" $HBA $WWN $STATE $SPEED 2>/dev/null
	#detect emulex cards
	if [[ 0 -lt $(ls /sys/class/scsi_host/host*/ | grep lpfc | wc -l 2>/dev/null) ]]	
	then
